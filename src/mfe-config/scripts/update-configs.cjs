...
const validateTsConfig = () => {
  ...
  // 1) Extract MFE name from package.json
  const pkgPath = path.join(process.cwd(), 'package.json');
  let mfeName = '';
  if (fs.existsSync(pkgPath)) {
    try {
      const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf-8'));
      mfeName = String(pkg.name).replace(/^mfe-/, '');
      console.log(chalk.green(`Detected MFE name: ${mfeName}`));
    } catch (err) {
      console.log(chalk.red(`Error parsing package.json: ${err}`));
    }
  }
  ...
  tsConfigFiles.forEach((configFile) => {
    ...
    if (!fs.existsSync(templatePath)) {
      console.log(chalk.yellow(`  ↳ no template, skipping`));
      return;
    }

    // For tsconfig.app.json we need to inject the MFE name
    if (configFile === 'tsconfig.app.json') {
      // 2) Read & replace placeholder in template
      let templateRaw = fs.readFileSync(templatePath, 'utf-8');
      if (mfeName) {
        templateRaw = templateRaw.replace(
          new RegExp('{<MFE NAME>}', 'g'),
          mfeName
        );
      }
      // 3) If no existing file, just write replaced template
      if (!fs.existsSync(targetPath)) {
        console.log(chalk.yellow(`  ↳ creating from template…`));
        fs.writeFileSync(targetPath, templateRaw, 'utf-8');
        console.log(chalk.green(`  ↳ created ${configFile}`));
      } else {
        // 4) Compare hashes of existing vs new content
        ...
        const newHash = require('crypto')
          ...
          .digest('hex');
        if (oldHash !== newHash) {
          console.log(chalk.yellow(`  ↳ differs, updating…`));
          fs.writeFileSync(targetPath, templateRaw, 'utf-8');
      ...
      // done with tsconfig.app.json
      return;
    }

    // For the other tsconfigs, same copy/compare logic as before:
    if (!fs.existsSync(targetPath)) {
      console.log(chalk.yellow(`  ↳ creating from template…`));
