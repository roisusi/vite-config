        ...
        return {
            BlockStatement(node) {
                ...
                if (tokenAfter && (
                    ...
                    tokenAfter.value === ',' || 
                    tokenAfter.value === '}' || 
                    tokenAfter.value === 'else' || 
                    tokenAfter.value === 'catch'
                )) {
                    return;
                }
...
export const requireTodoInComments = {
    meta: {
        type: 'suggestion',
        docs: {
            description: 'Enforce that all comments must include the word "todo" (case-insensitive).',
            category: 'Stylistic Issues',
            recommended: false,
        },
            ...
            Program() {
                ...
                comments.forEach(comment => {
                    ...
                    if (!/todo/i.test(comment.value) && 
                        !/eslint/i.test(comment.value) && 
                        !/^\/\*{1,2}/.test(getBlocks)) {
                        context.report({
                            loc: comment.loc,
                            message: 'Comment must include "TODO" (case-insensitive), "eslint" or blocks like /** and /* .',
                        });
                    }
                });
            },
...
export const requireUnionTypeInAlias = {
    meta: {
        type: 'suggestion',
        docs: {
            description: 'Type alias must not be defined as an object literal type. Use non-object or union types instead.',
            category: 'Stylistic Issues',
            recommended: false,
        },
        schema: [], // no options
